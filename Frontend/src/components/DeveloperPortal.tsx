...
export default function DeveloperPortal({ onBack }: DeveloperPortalProps) {
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white text-black rounded-full text-sm font-medium hover:bg-gray-100 transition-all duration-300"
            >
          ...
          {apiKeys.length === 0 ? (
            <div className="bg-black/30 rounded-3xl border border-black/40 p-12 text-center">
              <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                </svg>
              </div>
              <h3 className="text-xl font-light text-white mb-2">No API Keys Yet</h3>
              <p className="text-white/60 mb-6">Generate your first API key to start integrating with Seedora</p>
              <button
                onClick={() => setShowNewKeyModal(true)}
            ...
            <div className="space-y-4">
              {apiKeys.map((apiKey) => (
                <div key={apiKey._id} className="bg-black/30 rounded-3xl border border-black/40 p-6 hover:bg-black/40 transition-colors">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h3 className="text-lg font-medium text-white mb-2">{apiKey.name}</h3>
        ...
        <div className="mb-8">
          ...
          <div className="bg-black/30 rounded-3xl border border-black/40 p-6">
            <p className="text-white/70 mb-4">Make your first API call in seconds:</p>
            <div className="bg-black/60 rounded-2xl p-4 mb-4 relative group">
              <button
                onClick={() => copyToClipboard(`curl -X POST https://api.seedora.com/v1/predict \\
  -H "X-API-Key: YOUR_API_KEY" \\
  -F "image=@foot_thermogram.png"`)}
                className="absolute top-4 right-4 p-2 bg-white/10 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"
                title="Copy code"
              >
  ...
  -F "image=@foot_thermogram.png"`}
            ...
            <button className="px-6 py-2 bg-white/10 text-white rounded-full text-sm hover:bg-white/20 transition-colors">
        ...
        {/* Usage Statistics */}
        <div>
          <h2 className="text-2xl font-light text-white mb-6">Usage Statistics</h2>
          {usageStats ? (
      ...
      {showNewKeyModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-black/90 backdrop-blur-xl rounded-3xl border border-white/10 p-8 max-w-md w-full">
            <h3 className="text-2xl font-light text-white mb-4">Generate New API Key</h3>
            <p className="text-white/60 text-sm mb-6">Give your API key a descriptive name to help you identify it later.</p>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full rounded-2xl bg-white/5 border border-white/20 focus:border-white/40 focus:ring-1 focus:ring-white/20 outline-none px-4 py-3 text-white placeholder-white/50 text-sm"
                autoFocus
              />
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-6 py-3 bg-white text-black rounded-2xl text-sm font-medium hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Generate Key
              </button>
              <button
                ...
                className="flex-1 px-6 py-3 bg-white/10 text-white rounded-2xl text-sm hover:bg-white/20 transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      {newlyCreatedKey && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-black/90 backdrop-blur-xl rounded-3xl border border-white/10 p-8 max-w-lg w-full">
            <div className="text-center mb-6">
              ...
              <h3 className="text-2xl font-light text-white mb-2">API Key Created!</h3>
              <p className="text-white/60 text-sm">Save this key now. You won't be able to see it again.</p>
            </div>

            <div className="bg-black/60 rounded-2xl p-4 mb-6">
            ...
            <button
              ...
              className="w-full px-6 py-3 bg-white text-black rounded-2xl text-sm font-medium hover:bg-gray-100 transition-colors"
            >
              I've Saved My Key
            </button>
          </div>
